...
const express = require('express')
  ...
  , imageUploadDomain = 'http://localhost'

//      ╔══════════════════════════════════════╗
//      ║            REQUIRE FILES             ║
//      ╚══════════════════════════════════════╝
const usersCtrl = require('./controllers/usersCtrl')
  , dbCtrl = require('./controllers/dbCtrl')
  , sizesCtrl = require('./controllers/sizesCtrl')
  , matsCtrl = require('./controllers/materialsCtrl')
  , shipCtrl = require('./controllers/shippingTypeCtrl')
  , itemsCtrl = require('./controllers/itemsCtrl')
  , statesCtrl = require('./controllers/statesCtrl')
  , countryCtrl = require('./controllers/countryCtrl')
  , addressCtrl = require('./controllers/addressCtrl')
  , ordersCtrl = require('./controllers/ordersCtrl')
  , orderItemsCtrl = require('./controllers/orderItemsCtrl')
  , itemMatsCtrl = require('./controllers/itemMatsCtrl')
  , itemSizesCtrl = require('./controllers/itemSizesCtrl')
...
app.use(function(req, res, next) { //allow cross origin requests
  ...
  res.header("Access-Control-Allow-Origin", imageUploadDomain);
...
const getFilename = (name) => {
  let splitPath = imageUploadPath.split('')
  splitPath.shift()
  let path = splitPath.join('')
  itemImageUrl = `${imageUploadDomain}:${port}${path}${name}`
}
var upload = multer({
  storage: storage
}).single('file');
...
app.post('/upload', (req, res) => {
  upload(req, res, function(err){
      ...
      res.json({error_code:0, err_desc:null, url:itemImageUrl})
  })
})
